...
# Global set to track unique error objects for deduplication
unique_error_objects = set()
...
def initialize_error_json():
    ...
    try:
        ...
        # Clear the unique error objects set for fresh execution
        global unique_error_objects
        unique_error_objects.clear()
...
def save_error_to_json(error_detail):
    """Save error to single JSON file in real-time with object-level deduplication"""
    try:
        import json
        
        # Create a hashable representation of the error for deduplication
        error_key = (
            error_detail.get('error_type', ''),
            error_detail.get('error_message', '')[:100],
            error_detail.get('filename', ''),
            error_detail.get('line_number', ''),
            error_detail.get('column_number', ''),
            error_detail.get('dom_path', '')
        )
        
        # Check if this exact error already exists in set
        if error_key in unique_error_objects:
            print(f"🔄 Duplicate error object skipped: {error_detail.get('error_type', 'unknown')} - {error_detail.get('error_message', '')[:50]}...")
            return
        
        # Add to unique error objects set
        unique_error_objects.add(error_key)
...
def generate_error_signature(error_type, error_message, filename=None, lineno=None, colno=None):
    ...
    # Create a signature based on error type, message, and location
    signature_parts = [
        error_type,
        error_message[:100] if error_message else "",  # Limit message length
        filename or "",
